...
(() => {
  ...
  canvas.addEventListener('mousedown', (e) => {
    if (e.button === 0) {
      mouse.down = true;
      if (roundActive && !paused) { player.aim = angleTo(player, screenToWorld(mouse.x, mouse.y)); throwKnife(); }
    }
  });
  ...
  canvas.addEventListener('touchstart', (e) => {
    ...
    // Update player.aim synchronously (not just mouse.x/y) so this tap's throw --
    // fired immediately below, before the next update() tick would otherwise
    // recompute it -- already matches where you just tapped.
    player.aim = angleTo(player, screenToWorld(mouse.x, mouse.y));
  ...
  function worldTransform() {
    ...
    if (isTouch && player) {
      ...
      // camX/camY must target the actual SCREEN center (VIEW.w/2, VIEW.h/2) --
      // using a WORLD-space anchor (WORLD.w*scale/2) here was a longstanding bug:
      // it only happened to land near screen-center when scale ~= VIEW.w/WORLD.w,
      // and got dramatically worse once the mobile zoom floor (coverScale, above)
      // pushed scale much higher on tall/narrow phones -- the player ended up
      // pinned near/beyond the screen edge almost everywhere except exact map
      // center, instead of staying centered wherever the world bounds allow it.
      const camX = VIEW.w / 2 - player.x * scale;
      const camY = VIEW.h / 2 - player.y * (1 - camTilt) * scale;
  ...
  function startRound() {
    ...
    if (currentMode === 'murderer') {
      ...
      const mp = pts[count];
      murderer = { x: mp.x, y: mp.y, r: 16, alive: true, dir: rand(0, Math.PI*2), recharge: 0, wander: rand(0,Math.PI*2), wt: rand(40,90), speedMul: diff.aiSpeedMul };
    }
  ...
  function throwKnife() {
    ...
    // Always throw toward player.aim -- the SAME value the held-knife sprite and
    // the vision cone are drawn with -- instead of independently recomputing from
    // mouse.x/y. Recomputing separately here used to let the two disagree: e.g. on
    // touch, moving via the joystick correctly turns player.aim to face your
    // movement direction, but mouse.x/y only changes when you tap the canvas --
    // so hitting SHOOT without a fresh tap threw toward a stale/unrelated point
    // instead of the direction you were visibly aiming/moving.
  ...
  function update() {
    ...
    if (isTouch && joy.active && (joy.dx || joy.dy)) {
      ...
      // desktop mouse (kept live by mousemove), or touch without joystick movement
      // (kept live by the right-side canvas touchstart/touchmove handlers, which
      // update player.aim directly for the same reason -- see throwKnife()).
    ...
    if (murderer && murderer.alive) {
      ...
      // Face the direction actually being moved -- replaces the old periodic random
      // turn, which was fully disconnected from movement/throw direction and made
      // the character visibly face one way while moving/throwing another.
      murderer.dir = goalAng;
      ...
      if (murderer.recharge > 0) murderer.recharge--;
        ...
        murderer.dir = ka; // snap to face the throw direction at the moment of throwing
